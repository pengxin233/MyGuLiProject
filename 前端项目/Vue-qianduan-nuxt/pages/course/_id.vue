          ...
          <ol class="thr-attr-ol">
            ...
            <li>
              ...
              <aside>
                ...
                <br>
                <h6 class="c-fff f-fM mt10">{{(courseWebVo.viewCount)/2-0.5}}</h6>
              </aside>
            </li>
          </ol>
        </aside>
                  ...
                  <ul>
                    <li class="unBr">
                      <aside class="noter-pic">
                        <img v-if="user == null" width="50" height="50" class="picImg" src="~/assets/img/avatar-boy.gif">
                        <img v-if="user != null" width="50" height="50" class="picImg" :src="user.avatar">
                        </aside>
                      <div class="of">
                        <section class="n-reply-wrap">
                          <fieldset>
                            <textarea name="" v-model="mcomment.content" placeholder="输入您要评论的文字" id="commentContent"></textarea>
                          </fieldset>
...
export default {
  ...
  created(){
    this.updateViewCount()//更新浏览数
    this.getCourseInfo()
    this.getuserInfo() //得到用户登录信息
    this.gotoPage(1) //得到分页数据
  },
  methods:{
    //得到课程详情信息
    getCourseInfo(){
    ...
    addComment(){
      ...
      //封装课程评论信息
      this.mcomment.courseId = this.courseId
      this.mcomment.teacherId = this.courseWebVo.teacherId

      commentApi.saveCompent(this.mcomment)
        .then(response=>{
          //清除输入框的内容
          this.mcomment.content = ''
          //跳转到评论的最后一页，
          this.gotoPage(Math.floor((this.data.total/this.limit)+1))
        })
    },
    //得到用户登录信息
    getuserInfo(){
       var userStr = cookie.get('guli_ucenter') //得到的为json字符串
       if(userStr){
         this.user = JSON.parse(userStr) //该方法可以将字符串修改为json对象
         this.mcomment.memberId = this.user.id

         this.mcomment.avatar = this.user.avatar
         this.mcomment.nickname = this.user.nickname
       }
    },
    gotoPage(page){
    ...
    //浏览课程后更新课程的浏览数
    updateViewCount(){
      courseApi.updateViewCount(this.courseId)
    }
  }
};
</script>
