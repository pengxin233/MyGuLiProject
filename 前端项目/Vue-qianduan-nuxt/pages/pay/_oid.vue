...
export default {
    asyncData({ params, error }) {
        return orderApi.createNative(params.oid)
            .then(response=>{
                return {
    ...
    mounted(){
        ...
        this.time1 = setInterval(()=>{
            this.querryOrderStatus(this.payObj.out_trade_no,this.payObj.course_id)
        },3000)

    },
    methods:{
        querryOrderStatus(orderNo,courseId){
            orderApi.queryPayStatus(orderNo,courseId)
                .then(response=>{
                    //清除定时器
